...
describe('Синхронизация тип нагревателя ↔ модель (ручной режим)', () => {
  const seExists = (n: string) => Object.keys((db as any).models).includes(n)
    && (db as any).models[n].type === 'supply_exhaust';
  const supExists = (n: string) => Object.keys((db as any).models).includes(n)
    && (db as any).models[n].type === 'supply';

  it('heaterTypeOfModel определяет нагрев по имени', async () => {
    const { heaterTypeOfModel } = await import('../src/engine/selectModel');
    expect(heaterTypeOfModel('Unimax_P_CE')).toBe('электрический');
    expect(heaterTypeOfModel('Unimax_P_CW')).toBe('водяной');
    expect(heaterTypeOfModel('CAU_W')).toBe('водяной');
    expect(heaterTypeOfModel('CAU_F')).toBe('электрический');
    expect(heaterTypeOfModel('Nova')).toBe('без нагревателя');
  });

  it('Unimax_P_CE + водяной → Unimax_P_CW и обратно (рекуператор/исполнение сохранены)', async () => {
    const { modelForHeaterType } = await import('../src/engine/selectModel');
    expect(modelForHeaterType('Unimax_P_CE', 'водяной', seExists)).toBe('Unimax_P_CW');
    expect(modelForHeaterType('Unimax_P_CW', 'электрический', seExists)).toBe('Unimax_P_CE');
    expect(modelForHeaterType('Unimax_P_SE_EC', 'водяной', seExists)).toBe('Unimax_P_SW_EC');
  });

  it('приточные: CAU_F↔CAU_W, ECO_Slim↔ECO_Slim_W', async () => {
    const { modelForHeaterType } = await import('../src/engine/selectModel');
    expect(modelForHeaterType('CAU_F', 'водяной', supExists)).toBe('CAU_W');
    expect(modelForHeaterType('CAU_W', 'электрический', supExists)).toBe('CAU_F');
    expect(modelForHeaterType('ECO_Slim', 'водяной', supExists)).toBe('ECO_Slim_W');
  });

  it('смена нагревателя реально меняет подобранную позицию', () => {
    const base = {
      installation_type: 'приточно-вытяжная' as const, selection_mode: 'вручную' as const,
      manual_size_no: 1, flow: 500, head: 150, t_outdoor: -30, rh_outdoor: 80, t_supply: 21,
      t_indoor: 18, rh_indoor: 40, recup_type: 'пластинчатый' as const,
    };
    const e = runSelection({ ...base, manual_model_se: 'Unimax_P_CE', heater_type: 'электрический' });
    const w = runSelection({ ...base, manual_model_se: 'Unimax_P_CW', heater_type: 'водяной',
      t_water_in: 80, t_water_out: 60 });
    expect(e.modelName).toBe('Unimax_P_CE');
    expect(w.modelName).toBe('Unimax_P_CW');
    expect(e.m61!.name).not.toBe(w.m61!.name);
  });
});
